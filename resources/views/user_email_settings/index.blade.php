    ...
    <div class="col-md-6 grid-margin stretch-card">
        <div class="card">
            <div class="card-body">              
                ...
                <form id="email-settings-form" class="forms-sample" method="POST" action="{{ route('user_mail_settings.update', $mailSetting->id) }}">
                    ...
                    <form id="email-settings-form" class="forms-sample" method="POST" action="{{ route('user_mail_settings.store') }}">
                        ...
                        <div class="form-group">
                            ...
                            <input type="time" name="start_time" class="form-control" value="{{ old('start_time', isset($mailSetting)?$mailSetting->start_time:'') }}" required>
                            @error('start_time')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="time" name="end_time" class="form-control" value="{{ old('end_time', isset($mailSetting)?$mailSetting->start_time:'') }}" required>
                            @error('end_time')
                            <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                @if(isset($mailSetting))
                <form action="{{ route('user_mail_settings.destroy', $mailSetting->id) }}" id="delete-form" method="POST">
                    @csrf
                    @method('DELETE')
                    <div class="form-group">                            
                            <button type="button" id="delete" class="btn btn-danger me-2">Delete</button>
                    </div>
