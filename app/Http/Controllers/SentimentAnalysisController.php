...
class SentimentAnalysisController extends Controller
{
    ...
    {
        $user = Auth::user();
        $query = DB::table('user_mail_settings')
        ->where('user_id', '=', $user->id);          
               
        if ($query->count() > 0) {
            $getuserdata = $query->get();
        $imapPath = '{secure341.inmotionhosting.com:993/imap/ssl/novalidate-cert}INBOX';
        //foreach ($getuserdata as $data) {
        $username = $getuserdata[0]->primary_email;
        ...
        try{
            ...
            $responses['message'] = 'Unable to connect to the IMAP server';
        ...
        $max_timestamp = Mailcontent::where('email_id', $getuserdata[0]->primary_email)->max('mailrecvdate');
        ...
        $email_messages = array_slice($email_messages, 0, 10);
         ...
         if ($emailrecv_date > $max_timestamp) { //print_r($emailrecv_date);
             ...
             Mailcontent::create([
                 ...
                 'body' => $text,
                 'email_id'=>$getuserdata[0]->primary_email
             ]);
         }
         }
